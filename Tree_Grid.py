...
class Tree_Grid_operations():
    def __init__(self,parent,tree,grid):
        self.parent=parent
        self.tree=tree
        self.grid=grid
    ...
    def Selection_item(self, item_1):  
        ...
        if (str_4[0]=="Profile"):
            ...
            self.parent.EL.active_profile=int(str_4[1])
    ...
    def Add_tree_node(self,node):
        ...
        if (resultat[0]==-1):
            self.parent.Message("Error: no branch of Node inputs")
            return
    ...
    def Add_tree_beam(self,beam):
        ...
        if (resultat[0]==-1):
            self.parent.Message("Error: No branch of beam inputs\n")
            return
    ...
    def Remove_tree_node(self,numero):
        Item1=self.Search_item_tree_ctrl_perso("Nodes",numero)
        if Item1[0]==1:
            self.parent.GUI.m_treeCtrl1.Delete(Item1[1])
        else:
            self.parent.Message("Error : Point not found in the tree\n")

    
    def Remove_tree_beam(self,numero):
        Item1=self.Search_item_tree_ctrl_perso("Beams",numero)
        if Item1[0]==1:
            self.parent.GUI.m_treeCtrl1.Delete(Item1[1])
        else:
            self.parent.Message("Error : Element not found in the tree\n")
        
    
    def Modify_TG_from_cell(self):
        ...
        if (RowLabel=="modulus(GPa)"):
            ...
            if resultat[0]==1:
                ...
                self.parent.Message_dialog("None entry found in tree ctrl",wx.ICON_ERROR)
        else:
    ...
    def TreeCtrl_init(self):
        ...
        Tree_Lattice_Id=self.parent.GUI.m_treeCtrl1.AddRoot("Lattice")
        ProfilId=self.parent.GUI.m_treeCtrl1.AppendItem(Tree_Lattice_Id,"Profiles:")
        BasisId=self.parent.GUI.m_treeCtrl1.AppendItem(Tree_Lattice_Id, "Basis:")
        NodesId=self.parent.GUI.m_treeCtrl1.AppendItem(Tree_Lattice_Id, "Nodes:")
        BeamsId=self.parent.GUI.m_treeCtrl1.AppendItem(Tree_Lattice_Id, "Beams:")
        self.parent.GUI.m_treeCtrl1.AppendItem(BasisId, "Y.1:1,0")
        self.parent.GUI.m_treeCtrl1.AppendItem(BasisId, "Y.2:0,1")
        self.parent.GUI.m_treeCtrl1.AppendItem(NodesId, "N.1:0,0")
        self.parent.GUI.m_treeCtrl1.AppendItem(ProfilId, "Profile.1:210,0.3,rect,0.1")
